...
simulate GitLab CI vs no-CI contexts.
"""
...
class TestCommitRefName:
    ...
    def test_returns_gitlab_name(self, monkeypatch: pytest.MonkeyPatch):
...
class TestCommitShortSha:
    ...
    def test_derives_from_full_sha_when_short_unset(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        # When only the full SHA is set, slice the first 8 chars.
...
class TestCommitBeforeSha:
    ...
    def test_returns_gitlab_value(self, monkeypatch: pytest.MonkeyPatch):
...
class TestRegistryImage:
    ...
    def test_returns_value(self, monkeypatch: pytest.MonkeyPatch):
        path = "registry.example.com/example-org/helm-charts"
