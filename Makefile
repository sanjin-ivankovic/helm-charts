...
# Use CI variables if available (CI_REGISTRY, CI_REGISTRY_IMAGE, etc. are the
# standard GitLab CI env names), otherwise fall back to defaults.
...
# OCI registry path: oci://<host>/<owner>/<project>
...
help: ## Show this help message
	...
	@echo "  make release CHART=it-tools          # Full workflow for it-tools"
...
setup: ## Install pre-commit hooks and development dependencies
	...
	@echo "$(COLOR_GREEN)Done. Pre-commit hooks are now active.$(COLOR_RESET)"
...
login: ## Login to OCI registry
	...
	@echo "$(COLOR_YELLOW)Note: For Harbor, use a robot account or PAT with 'repository.pull' + 'repository.push' permissions as the password.$(COLOR_RESET)"
